...
/**
 * La classe Adiacente gestisce la logica delle mosse adiacenti,
 ...
 *  Similmente alla classe Spostamento,
 * Adiacente è una classe di controllo poiché gestisce e coordina le operazioni legate alle mosse adiacenti nel gioco.
 */
public class Adiacente extends MossaBase {
    private Tavoliere tavoliere;
    private List<String> logMosse;

    /**
     * Costruttore della classe Adiacente.
     * @param tavoliereadiacente il tavoliere corrente
     */
    public Adiacente(final Tavoliere tavoliereadiacente) {
        super(tavoliereadiacente);
        this.tavoliere = tavoliereadiacente;
    ...
    public boolean isMossaValida(final String from, final String to, final char giocatore) {
        ...
        // Verifica che la mossa sia adiacente
     ...
     * @param giocatore il giocatore corrente ('\u26C0' per il giocatore 1, '\u26C2' per il giocatore 2)
     */
    void logMossa(final String from, final String to, final char giocatore) {
        String mossa = "Giocatore " + (giocatore == '\u26C0' ? "1 (Bianco)" : "2 (Nero)") + ": " + from + "-" + to;
        logMosse.add(mossa);
        AdiacenteStampa.stampaMossaRegistrata(giocatore, from, to);
    }
     ...
     */
    int[] notazioneAlgebricaToCoord(final String notazione) {
    ...
    /**
     * Restituisce il log delle mosse.
     *
     * @return la lista delle mosse.
     */
    List<String> getLogMosse() {
        return logMosse;
    }
}
